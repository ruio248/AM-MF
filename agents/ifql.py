...
class IFQLAgent(flax.struct.PyTreeNode):
    ...
    def sample_actions(
        ...
        # Fix shape mismatch: ensure consistent dimensions for concatenation
        # Reshape observations and actions to have compatible shapes
        batch_size = observations.shape[0]
        obs_dim = observations.shape[-1]
        
        # Reshape observations: (batch_size, obs_dim) -> (num_samples, batch_size, obs_dim)
        n_observations = jnp.tile(jnp.expand_dims(observations, 0), (self.config['num_samples'], 1, 1))
        n_orig_observations = jnp.tile(jnp.expand_dims(orig_observations, 0), (self.config['num_samples'], 1, 1))
        
        # Reshape actions: (batch_size, num_samples, action_dim) -> (num_samples, batch_size, action_dim)
        actions = jnp.transpose(actions, (1, 0, 2))
        
        for i in range(self.config['flow_steps']):
            # Create time tensor with shape (num_samples, batch_size, 1)
            t = jnp.full((self.config['num_samples'], batch_size, 1), i / self.config['flow_steps'])
