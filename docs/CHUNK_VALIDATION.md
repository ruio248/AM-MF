...
最终 GPU 全量回归曾发现候选动作参考测试的数值问题：逐算子 eager 参考与完整
JIT 图在候选数 5 时相差 `4.6361e-6`，超过原断言 `1e-6`。独立诊断确认
候选数 1/2/5 的所选下标一致；将独立参考图也完整 JIT 后，三组与生产采样器的
最大绝对差均为 **0**。因此只修正测试的编译口径，保持原容差、不改采样实现。
初次日志 `final-unittest.log` 保留；修正后的全量复核另存
`final-unittest-after-jit-reference.log`，不覆盖初次失败记录。
